        ...
        <div class="widget" style="background: linear-gradient(135deg, var(--primary), #0A9396); color: white; border: none;">
            ...
            <div style="display: flex; flex-direction: column; gap: 1.25rem;">
                <div style="display: flex; align-items: center; gap: 1rem;">
                    <div style="width: 48px; height: 32px; background: white; border-radius: 4px; display: flex; align-items: center; justify-content: center; color: #1E293B; font-weight: 800; font-size: 0.7rem;">MANDIRI</div>
                    <div>
                        <div style="font-size: 1.1rem; font-weight: 700; letter-spacing: 1px;">1234-5678-9012</div>
                        <div style="font-size: 0.75rem; opacity: 0.8;">a.n Wisma AAM KOSQU</div>
                    </div>
                </div>
                <div style="display: flex; align-items: center; gap: 1rem;">
                    <div style="width: 48px; height: 32px; background: white; border-radius: 4px; display: flex; align-items: center; justify-content: center; color: #003399; font-weight: 800; font-size: 0.7rem;">BCA</div>
                    <div>
                        <div style="font-size: 1.1rem; font-weight: 700; letter-spacing: 1px;">9876-5432-10</div>
                        <div style="font-size: 0.75rem; opacity: 0.8;">a.n Wisma AAM KOSQU</div>
                    </div>
                </div>
            </div>
            <div style="margin-top: 2rem; padding-top: 1.5rem; border-top: 1px solid rgba(255,255,255,0.1); font-size: 0.8rem; line-height: 1.6; opacity: 0.9;">
                ...
                Pastikan nominal transfer sesuai dengan harga sewa kamar Anda untuk mempercepat proses validasi.
            </div>
        </div>
    ...
    <div class="widget">
        @if($isPaid)
            <div style="text-align: center; padding: 4rem 2rem;">
                ...
                <h2 style="color: var(--text-main); font-weight: 800; margin-bottom: 0.5rem;">Tagihan Sudah Terbayar</h2>
                <p style="color: var(--text-muted); font-size: 0.95rem; line-height: 1.6;">Anda sudah melaporkan pembayaran untuk bulan <strong>{{ $currentMonth }}</strong>. Silakan cek status validasi di menu Invoice.</p>
        ...
        @else
            <form action="{{ route('user.payment.store') }}" method="POST" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 1.5rem;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                    <div>
                        <label style="display: block; font-size: 0.85rem; font-weight: 700; color: var(--text-muted); margin-bottom: 0.6rem;">Bulan Tagihan</label>
                        <input type="text" name="bulan_tagihan" value="{{ $currentMonth }}" readonly style="width: 100%; padding: 0.8rem; border-radius: 10px; border: 1px solid #E2E8F0; background: #F8FAFC; color: #64748B; font-weight: 600; outline: none;">
                    </div>
                    <div>
                        ...
                        <input type="number" name="jumlah_bayar" value="{{ (int)$kamar->harga_sewa }}" required style="width: 100%; padding: 0.8rem; border-radius: 10px; border: 1px solid #E2E8F0; font-weight: 700; color: var(--primary); outline: none;">
                    </div>
                </div>
